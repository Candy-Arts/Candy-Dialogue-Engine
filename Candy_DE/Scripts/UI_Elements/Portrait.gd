...
func receive_portrait(character_ref: String, portrait_raw: String, play_portrait: String) -> void:
	...
	if portrait_node is AnimatedSprite2D or portrait_node is AnimatedSprite3D:
		...
		character_frames = candy_de.portraits_folder.path_join(character_ref).path_join("Sprite_Frames").path_join(character_ref).path_join(".tres")
		animation = resolved
...
func texturebutton_portrait(portrait_file: String) -> void:
	...
	#% The character's portrait folder is the parent of the portrait file.
	#% Subfolder name matches the file base + underscore + extension, e.g. "Alice_png".
	var character_folder = portrait_file.get_base_dir()
	var file_base = portrait_file.get_file().get_basename()
	var file_ext = portrait_file.get_extension().to_lower()
	var button_folder = character_folder.path_join("Button_Textures").path_join(file_base + "_" + file_ext)

	if not DirAccess.dir_exists_absolute(button_folder):
		return

	#@ Scan the subfolder once and assign files to their matching texture slots:
	var texture_slots = [
		"texture_pressed",
		"texture_hover",
		"texture_disabled",
