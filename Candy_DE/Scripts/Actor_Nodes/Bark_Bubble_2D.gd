...
#^ Required Node Paths:
#? If you use your own custom node hierarchy, update this appropriately.
@onready var label: RichTextLabel = get_node("SubViewport/Text")	#/ The node that displays text. [REQUIRED BY THE ENGINE]
@onready var sprite: Sprite2D = get_node("Sprite2D")				#/ The Sprite3D node. [Unused]
...
@export var max_width = 256								#/ Maximum width the bubble can reach.
...
func show_text(spoken_text):
	...
	self.position.y = -y_offset - label.size.y / 2
