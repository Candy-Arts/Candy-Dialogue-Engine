...
func set_bust(actor: String, bust_ref: String, file_ref: String, anim_name: String, loop: int, wait: int, time_target: float) -> void:
	...
	if bust_node is TextureButton:
		...
		var file_ext = file_path.get_extension().to_upper()
		var file_base = file_path.get_file().get_basename()
		var folder_with_ext = actor_folder.path_join("TB_" + file_base + "_" + file_ext)
		var folder_without_ext = actor_folder.path_join("TB_" + file_base)

		var button_folder = ""
		if DirAccess.dir_exists_absolute(folder_with_ext) or DirAccess.dir_exists_absolute(folder_with_ext.to_lower()):
			button_folder = folder_with_ext if DirAccess.dir_exists_absolute(folder_with_ext) else folder_with_ext.to_lower()
		elif DirAccess.dir_exists_absolute(folder_without_ext) or DirAccess.dir_exists_absolute(folder_without_ext.to_lower()):
			button_folder = folder_without_ext if DirAccess.dir_exists_absolute(folder_without_ext) else folder_without_ext.to_lower()
...
func _on_play_jml(event: String, actor: String, bust_node_new: Node, bust_node_old: Node) -> void:
	...
	match event:
		"Join":
			if candy_de.actors.has(actor) and candy_de.actors[actor].has("VN Join Effect"):
				jml_method = candy_de.actors[actor]["VN Join Effect"][0]
				jml_value = candy_de.actors[actor]["VN Join Effect"][1]
			if jml_method == "":
		...
		"Move_From":
			if candy_de.actors.has(actor) and candy_de.actors[actor].has("VN Move From Effect"):
				jml_method = candy_de.actors[actor]["VN Move From Effect"][0]
				jml_value = candy_de.actors[actor]["VN Move From Effect"][1]
			if jml_method == "":
		...
		"Move_To":
			if candy_de.actors.has(actor) and candy_de.actors[actor].has("VN Move To Effect"):
				jml_method = candy_de.actors[actor]["VN Move To Effect"][0]
				jml_value = candy_de.actors[actor]["VN Move To Effect"][1]
			if jml_method == "":
		...
		"Leave":
			if candy_de.actors.has(actor) and candy_de.actors[actor].has("VN Leave Effect"):
				jml_method = candy_de.actors[actor]["VN Leave Effect"][0]
				jml_value = candy_de.actors[actor]["VN Leave Effect"][1]
			if jml_method == "":
